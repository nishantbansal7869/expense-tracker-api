...
public class CategoryRepositoryImpl implements CategoryRepository{
    ...
    private static final String SQL_FIND_ALL_CATEG0RIES = "SELECT C.CATEGORY_ID, C.USER_ID, C.TITLE, C.DESCRIPTION," +
            " COALESCE(SUM(T.AMOUNT), 0) TOTAL_EXPENSE" +
            " FROM ET_TRANSACTIONS T RIGHT OUTER JOIN ET_CATEGORIES C ON C.CATEGORY_ID = T.CATEGORY_ID" +
    ...
    private static final String SQL_DELETE_CATEGORY = " DELETE FROM ET_CATEGORIES WHERE USER_ID = ? AND" +
            " CATEGORY_ID = ?";
    ...
    public List<Category> findAll(Integer userId) throws EtResourceNotFoundException {
        return jdbcTemplate.query(SQL_FIND_ALL_CATEG0RIES, new Object[]{userId}, categoryRowMapper);
    }
